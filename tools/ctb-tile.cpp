...
public:
  TerrainBuild(const char *name, const char *version)
      : Command(name, version), outputDir("."), outputFormat("Terrain"),
        profile("geodetic"), threadCount(-1), tileSize(0), startZoom(-1),
        endZoom(-1), verbosity(1), resume(false), meshQualityFactor(1.0),
        metadata(false), cesiumFriendly(false), vertexNormals(false) {}
  ...
  void check() const {
    ...
    case 0:
      ...
      break;
    default:
      cerr << "  Error: Only one command line argument must be specified"
           << endl;
      break;
    }
  ...
  static void setOutputDir(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->outputDir =
        command->arg;
  }

  static void setOutputFormat(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->outputFormat =
        command->arg;
  }
  ...
  static void setThreadCount(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->threadCount =
        atoi(command->arg);
  }

  static void setTileSize(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->tileSize =
        atoi(command->arg);
  }

  static void setStartZoom(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->startZoom =
        atoi(command->arg);
  }

  static void setEndZoom(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->endZoom =
        atoi(command->arg);
  }
  ...
  static void setResampleAlg(command_t *command) {
    ...
    static_cast<TerrainBuild *>(Command::self(command))
        ->tilerOptions.resampleAlg = eResampleAlg;
  }

  static void addCreationOption(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))
        ->creationOptions.AddString(command->arg);
  }

  static void setErrorThreshold(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))
        ->tilerOptions.errorThreshold = atof(command->arg);
  }

  static void setWarpMemory(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))
        ->tilerOptions.warpMemoryLimit = atof(command->arg);
  }
  ...
  static void setMeshQualityFactor(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->meshQualityFactor =
        atof(command->arg);
  }
...
static int CPL_STDCALL verboseProgress(double dfComplete,
...
int showProgress(int currentIndex, string filename) {
  ...
  return progressFunc(currentIndex / (double)iteratorSize, message.c_str(),
                      NULL);
}
...
static bool fileCopy(const std::string &sourceFilename,
  ...
  void writeJsonFile(const std::string &filename,
    ...
    for (size_t i = 0, icount = levels.size(); i < icount; i++) {
      ...
      if (level.finalX >= level.startX) {
        fprintf(fp,
                "{ \"startX\": %li, \"startY\": %li, \"endX\": %li, \"endY\": "
                "%li }",
                long(level.startX), long(level.startY), long(level.finalX),
                long(level.finalY));
      }
      fprintf(fp, " ]\n");
    }
...
static std::string createEmptyRootElevationFile(std::string &fileName,
                                                const Grid &grid,
  ...
  double adfGeoTransform[6] = {tileBounds.getMinX(), resolution, 0,
                               tileBounds.getMaxY(), 0,          -resolution};
  ...
  fileName += ".tif";
  GDALDataset *poDataset = poDriver->Create(fileName.c_str(), tileSize,
                                            tileSize, 1, GDT_Float32, nullptr);
  ...
  if (heightsBand->RasterIO(GF_Write, 0, 0, tileSize, tileSize,
                            (void *)rasterHeights, tileSize, tileSize,
...
static void buildGDAL(GDALSerializer &serializer, const RasterTiler &tiler,
                      TerrainBuild *command, TerrainMetadata *metadata) {
  GDALDriver *poDriver =
      GetGDALDriverManager()->GetDriverByName(command->outputFormat);
  ...
  if (poDriver->pfnCreateCopy == NULL) {
    throw CTBException("The GDAL driver must be write enabled, specifically "
                       "supporting 'CreateCopy'");
  }

  const char *extension = poDriver->GetMetadataItem(GDAL_DMD_EXTENSION);
  i_zoom startZoom = (command->startZoom < 0) ? tiler.maxZoomLevel()
                                              : command->startZoom,
  ...
  while (!iter.exhausted()) {
    ...
    if (serializer.mustSerializeCoordinate(coordinate)) {
      GDALTile *tile = *iter;
      serializer.serializeTile(tile, poDriver, extension,
...
static void buildTerrain(TerrainSerializer &serializer,
                         const TerrainTiler &tiler, TerrainBuild *command,
                         TerrainMetadata *metadata) {
  i_zoom startZoom =
      (command->startZoom < 0) ? tiler.maxZoomLevel() : command->startZoom;
...
// DEBUG Chunker:
  ...
  // if (destSRS.importFromEPSG(3857) != OGRERR_NONE) {
  //   throw CTBException("Could not create EPSG:3857 spatial reference");
  // }
  auto coordinateTransform =
      OGRCreateCoordinateTransformation(&iter.getGrid().getSRS(), &destSRS);
...
static void buildMetadata(const RasterTiler &tiler, TerrainBuild *command,
                          TerrainMetadata *metadata) {
  const string dirname = string(command->outputDir) + osDirSep;
  i_zoom startZoom = (command->startZoom < 0) ? tiler.maxZoomLevel()
                                              : command->startZoom,
 ...
 */
static int runTiler(const char *inputFilename, TerrainBuild *command,
                    Grid *grid, TerrainMetadata *metadata) {
  ...
  CTBFileTileSerializer serializer(string(command->outputDir) + osDirSep,
                                   command->resume);
  ...
  try {
    ...
    if (command->metadata) {
      ...
      const MeshTiler tiler(poDataset, *grid, command->tilerOptions,
                            command->meshQualityFactor);
      buildMesh(serializer, tiler, command, threadMetadata,
...
int main(int argc, char *argv[]) {
  ...
  command.option("-o", "--output-dir <dir>",
                 "specify the output directory for the tiles (defaults to "
                 "working directory)",
                 TerrainBuild::setOutputDir);
  command.option("-f", "--output-format <format>",
                 "specify the output format for the tiles. This is either "
                 "`Terrain` (the default), `Mesh` (Chunked LOD mesh), or any "
                 "format listed by `gdalinfo --formats`",
                 TerrainBuild::setOutputFormat);
  command.option("-p", "--profile <profile>",
                 "specify the TMS profile for the tiles. This is either "
                 "`geodetic` (the default) or `mercator`",
                 TerrainBuild::setProfile);
  command.option("-c", "--thread-count <count>",
                 "specify the number of threads to use for tile generation. On "
                 "multicore machines this defaults to the number of CPUs",
                 TerrainBuild::setThreadCount);
  command.option("-t", "--tile-size <size>",
                 "specify the size of the tiles in pixels. This defaults to 65 "
                 "for terrain tiles and 256 for other GDAL formats",
                 TerrainBuild::setTileSize);
  command.option("-s", "--start-zoom <zoom>",
                 "specify the zoom level to start at. This should be greater "
                 "than the end zoom level",
                 TerrainBuild::setStartZoom);
  command.option("-e", "--end-zoom <zoom>",
                 "specify the zoom level to end at. This should be less than "
                 "the start zoom level and >= 0",
                 TerrainBuild::setEndZoom);
  command.option("-r", "--resampling-method <algorithm>",
                 ...
                 TerrainBuild::setResampleAlg);
  command.option("-n", "--creation-option <option>",
                 "specify a GDAL creation option for the output dataset in the "
                 "form NAME=VALUE. Can be specified multiple times. Not valid "
                 "for Terrain tiles.",
                 TerrainBuild::addCreationOption);
  command.option("-z", "--error-threshold <threshold>",
                 "specify the error threshold in pixel units for "
                 "transformation approximation. Larger values should mean "
                 "faster transforms. Defaults to 0.125",
                 TerrainBuild::setErrorThreshold);
  command.option(
      "-m", "--warp-memory <bytes>",
      "The memory limit in bytes used for warp operations. Higher settings "
      "should be faster. Defaults to a conservative GDAL internal setting.",
      TerrainBuild::setWarpMemory);
  command.option("-R", "--resume", "Do not overwrite existing files",
                 TerrainBuild::setResume);
  command.option("-g", "--mesh-qfactor <factor>",
                 ...
                 TerrainBuild::setMeshQualityFactor);
  command.option("-l", "--layer", "only output the layer.json metadata file",
                 TerrainBuild::setMetadata);
  command.option(
      "-C", "--cesium-friendly",
      "Force the creation of missing root tiles to be CesiumJS-friendly",
      TerrainBuild::setCesiumFriendly);
  command.option("-N", "--vertex-normals",
                 "Write 'Oct-Encoded Per-Vertex Normals' for Terrain Lighting, "
                 "only for `Mesh` format",
                 TerrainBuild::setVertexNormals);
  ...
  if (VSIStatExL(command.outputDir, &stat,
    ...
    cerr << "Error: The output directory does not exist: " << command.outputDir
         << endl;
    return 1;
  } else if (!VSI_ISDIR(stat.st_mode)) {
    cerr << "Error: The output filepath is not a directory: "
         << command.outputDir << endl;
    return 1;
  }
  ...
  // CesiumJS friendly?
  if (command.cesiumFriendly && (strcmp(command.profile, "geodetic") == 0) &&
      command.endZoom <= 0) {

    // Create missing root tiles if it is necessary
    if (!command.metadata) {
      std::string dirName0 =
          string(command.outputDir) + osDirSep + "0" + osDirSep + "0";
      std::string dirName1 =
          string(command.outputDir) + osDirSep + "0" + osDirSep + "1";
      ...
      } else if (!fileExists(tileName0) && fileExists(tileName1)) {
      ...
      if (missingTileCoord.zoom != missingZoom) {
        ...
        command.startZoom = 0;
        command.endZoom = 0;
        missingTileName = createEmptyRootElevationFile(missingTileName, grid,
                                                       missingTileCoord);
  ...
  if (metadata) {
    ...
    if (std::string::npos != rfindpos)
      datasetName = datasetName.erase(rfindpos);

    metadata->writeJsonFile(
        filename, datasetName, std::string(command.outputFormat),
